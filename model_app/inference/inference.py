...
def predict_fn(input_data, model):
    ...
    if isinstance(input_data, dict) and input_data.get("ping"):
        print("[predict_fn] ping request detected")
        return {"ping": True}
...
def output_fn(prediction, accept):
    ...
    if isinstance(prediction, dict) and prediction.get("ping"):
        return json.dumps({"status": "ok", "model": "healthy"}), accept
